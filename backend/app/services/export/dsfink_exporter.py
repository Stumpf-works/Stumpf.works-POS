...
class DSFinVKExporter:
    ...
    def export(
        self, start_date: datetime, end_date: datetime, export_type: str = "full"
    ) -> bytes:
        """
        ...
        with tarfile.open(fileobj=tar_buffer, mode="w:gz") as tar:
            ...
            files = {
                ...
                "tse_transaktionen.csv": self._export_tse_transactions(
                    start_date, end_date
                ),
            }
    ...
    def _export_products(self) -> str:
        ...
        # Get products
        products = (
            self.db.query(Product).filter(Product.tenant_id == self.tenant_id).all()
        )
    ...
    def _export_transaction_headers(
        ...
        for txn in transactions:
            writer.writerow(
                [
                    ...
                    txn.tse_signature or "",
                    (
                        txn.tse_time_start.strftime("%Y-%m-%d %H:%M:%S")
                        if txn.tse_time_start
                        else ""
                    ),
                    (
                        txn.tse_time_end.strftime("%Y-%m-%d %H:%M:%S")
                        if txn.tse_time_end
                        else ""
                    ),
